    ...
    def __init__(self, in_size, out_size, is_batchnorm, dropout):
        ...
        if is_batchnorm:
            if dropout:
                ...
                self.conv2 = nn.Sequential(nn.Conv2d(out_size, out_size, 3, 1, 1), nn.BatchNorm2d(out_size), nn.ReLU(), nn.Dropout())
            else:
...
class unetUp(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, dropout):
        ...
        self.conv = unetConv2(in_size, out_size, True, dropout)
        if is_deconv:
